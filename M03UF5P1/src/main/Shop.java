...
//		shop.showSalesValue();
     ...
     * Read inventory file
     */
    public ArrayList<Product> loadInventory() throws IOException {
		...
		// Read data line by line if not empty
		while (line != null) {
			
			String[] primeraSeparacion = line.split(";");
			...
			for (int i = 0; i < primeraSeparacion.length; i++) {
				...
				switch (i) {
					case 0:
						product = segundaSeparacion[1];
						break;
					case 1:
						price = Double.parseDouble(segundaSeparacion[1]);
						break;
					case 2:
						stock = Integer.parseInt(segundaSeparacion[1]);
						break;
					default:
						break;
				}
			...
			// Insert every line's data as a product
			inventory.add(new Product(product, new Amount(price, "€"), true, stock));
			line = br.readLine();
		}
		fr.close();
		br.close();
